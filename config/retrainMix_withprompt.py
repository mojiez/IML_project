_base_ = './retrainMix.py'
# 修改导入方式
custom_imports = dict(
    imports=['models.backbones.swin_prompt'],  # 不要用IML_code前缀
...
model = dict(
    ...
    backbone=dict(
        type='PromptSwinTransformer',
        num_prompts=4,  # 添加的prompt数量
        prompt_dim=128,  # prompt维度，需要和patch_embed输出维度一致
...
# 模型检查点 按epoch保存 每1个epoch保存一次 不保存优化器状态（每个epoch结束时保存一次模型权重）
...
work_dir = './train_checkpoint/1103_prompt_v1'
device = 'cuda'
gpu_ids = [1]
